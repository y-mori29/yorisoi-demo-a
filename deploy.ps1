...
$EnvFile = "deploy_env_demo_a.yaml"
...
if ($Target -eq "all" -or $Target -eq "backend") {
    ...
    # Submit Build
    cmd /c "gcloud builds submit backend --tag gcr.io/$ProjectId/$Environment-backend"
    
    # Deploy to Cloud Run
    # Use environment-specific variables file
    cmd /c "gcloud run deploy $Environment-backend --image gcr.io/$ProjectId/$Environment-backend --platform managed --region $Region --allow-unauthenticated --port 8080 --env-vars-file $EnvFile"
...
"@
    ...
    cmd /c "gcloud builds submit frontend --config frontend/cloudbuild.temp.yaml"
    Remove-Item frontend/cloudbuild.temp.yaml
    
    # Deploy
    cmd /c "gcloud run deploy $Environment-frontend --image gcr.io/$ProjectId/$Environment-frontend --platform managed --region $Region --allow-unauthenticated --port 8080"
...
"@
    ...
    cmd /c "gcloud builds submit dashboard --config dashboard/cloudbuild.temp.yaml"
    Remove-Item dashboard/cloudbuild.temp.yaml
    
    # Deploy
    cmd /c "gcloud run deploy $Environment-dashboard --image gcr.io/$ProjectId/$Environment-dashboard --platform managed --region $Region --allow-unauthenticated --port 8080"
